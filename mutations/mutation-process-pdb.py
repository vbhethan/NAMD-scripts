...
parser.add_argument("--mutation", help="Mutation code: format is {ID}{Three letter amino acid code}")
...
$sel writepsf {name}.solvated.psf
$sel writepdb {name}.solvated.pdb

package require mutator

mutator -psf {name}.solvated.psf - pdb {name}.solvated.pdb -o {name}.{mutation}.final -resid {mutInd} -mut {mutCode} -FEP
